...
double** GS_red_black_iteration(double delta,int local_n_x, int n_y,double *y, double** local_phi_upd, double** q, int num_pros, int proc_id, double* shdw_pts_up, double* shdw_pts_dwn)
{
...
int main(int argc, char** argv)
{
    ...
    if(proc_id==0)
    {
        ...
        while(abs(curr_err) > err_thresh && iter_count < iter_lim)
        {
            ...
            local_phi_upd = GS_red_black_iteration(delta, local_n_x, n_y, y ,local_phi_upd,local_q, num_pros, proc_id, shdw_pts_dwn, shdw_pts_up);
        ...
        string filename_mat = "Poisson_GS_red_black_MPI_"+ to_string(num_pros) +"_"+delta_s+".csv";
        write_to_csv(delta,n_x,n_y,x,y,phi_upd,filename_mat);

        string filename_vec = "Poisson_GS_red_black_error_MPI_"+ to_string(num_pros) +"_"+delta_s+".csv";
        vector_csv(delta,iterations,errors,filename_vec);

        if(delta == 0.005)
        {
            string filename_time = "Poisson_GS_red_black_speedup.csv";
