...
{
    string file_name = "Poisson_GaussSeidel_RedBlack_data_grid_" + to_string(grid_size) + "_threads_" + to_string(thread) + ".csv";
...
{
    string file_name = "Poisson_GaussSeidel_RedBlack_data_grid_time_data.csv";
    ofstream dataFile(file_name);
    if (!dataFile.is_open()) 
    {
        cerr << "Error: Unable to open file for writing." << endl;
...
{
    string file_name = "Poisson_GaussSeidel_RedBlack_data_thread_time_data.csv";
    ofstream dataFile(file_name);
    if (!dataFile.is_open()) 
    {
        cerr << "Error: Unable to open file for writing." << endl;
...
double gauss_seidel_red_black_iteration(int n, double*x, double grid_size, double**& phi, double** actual_values, double** q, long long int iter_limit, double err_thresh, int thread, double* timer, double (*error_function)(int, double**,double**,int)) 
{
    ...
    while(fabs(error)>err_thresh && count < iter_limit)
    {
...
double red_black_method_timer(double grid_size, int thread) 
{
    double start = -1;
    double end = 1;
    double* x;
    ...
    double timer;
    gauss_seidel_red_black_iteration(n, x,grid_size, phi, actual_values, q, 10000000, 0.01, thread, &timer,&error_function);
...
int main() 
{
    ...
    double temp_time = red_black_method_timer(0.1, 2);
    ...
    for (int i=0;i<3 ; i++) 
    {
        ...
        double timer = red_black_method_timer(grid_sizes[i], num_threads[2]);
        grid_time[i]=timer;
    ...
    for (int i=0;i<3 ; i++) 
    {
        ...
        double timer = red_black_method_timer(grid_sizes[2], num_threads[i]);
        thread_time[i]=timer;
